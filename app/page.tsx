...
/* ── Stars helper (server-safe — no event handlers) ─────────── */
...
/* ── Page (Server Component — ZERO event handlers allowed) ───── */
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{ background: "linear-gradient(135deg, #f0fff0 0%, #e0f8f0 50%, #f0fff8 100%)", borderBottom: "1px solid #e8f5e8" }}
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-24">
          <div className="grid md:grid-cols-2 gap-12 items-center min-h-[520px]">
              ...
              <form action="/tasks/create" method="GET" className="mb-5">
                <div className="search-wrapper flex">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(135deg, #14A800, #00d4aa)", borderRadius: "18px", border: "none", cursor: "pointer" }}
                  >
            ...
            <div className="hidden md:flex items-center justify-center relative h-[480px]">
              <div className="absolute w-80 h-80 rounded-full blur-3xl opacity-50"
                   style={{ background: "radial-gradient(circle, rgba(0,212,170,.30), rgba(20,168,0,.15))" }} />

              {/* Card 1 */}
              <div className="absolute top-8 right-4 w-64 bg-white p-4 animate-float-slow"
                   style={{ borderRadius: "20px", border: "1px solid #e8f5e8", boxShadow: "0 8px 32px rgba(20,168,0,.14)", transform: "rotate(3deg)" }}>
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-lg font-extrabold price-gradient">150&thinsp;<span className="text-xs font-semibold text-gray-400">сом</span></span>
              ...
              {/* Card 3 */}
              <div className="absolute bottom-10 left-2 w-56 bg-white p-3.5"
                   style={{ borderRadius: "20px", border: "1px solid #e8f5e8", boxShadow: "0 8px 24px rgba(20,168,0,.12)", transform: "rotate(-2deg)" }}>
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-lg font-extrabold price-gradient">500&thinsp;<span className="text-xs font-semibold text-gray-400">сом</span></span>
              ...
              {/* AI badge */}
              <div className="absolute top-4 left-8 bg-white px-3.5 py-2.5 flex items-center gap-2"
                   style={{ borderRadius: "20px", border: "1px solid #e8f5e8", boxShadow: "0 4px 16px rgba(20,168,0,.20)" }}>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 relative">
            ...
            {[
              { n: "01", emoji: "📝", title: "Опишите задачу",      desc: "Напишите своими словами — как другу. AI сам заполнит категорию, бюджет и срок.", gradient: "linear-gradient(135deg, #14A800, #00d4aa)" },
              { n: "02", emoji: "👤", title: "Выберите исполнителя", desc: "AI подберёт топ помощников рядом. Смотрите рейтинг, отзывы — выбирайте.", gradient: "linear-gradient(135deg, #60a5fa, #6366f1)" },
              { n: "03", emoji: "✅", title: "Получите результат",   desc: "Деньги заморожены до выполнения. Исполнитель получает оплату только от вас.", gradient: "linear-gradient(135deg, #10b981, #00d4aa)" },
            ].map((item) => (
              <div key={item.n} className="relative z-10 text-center flex flex-col items-center">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl mb-4 shadow-lg"
                     style={{ background: item.gradient }}>{item.emoji}</div>
        ...
        <section className="py-16 px-4" style={{ background: "linear-gradient(135deg, #f0fff0 0%, #e8f8f0 100%)" }}>
          <div className="max-w-5xl mx-auto">
            <div className="flex justify-between items-end mb-10">
              ...
              <Link href="/tasks" className="text-[#14A800] hover:text-[#0d8c00] font-bold text-sm">Все задачи →</Link>
            </div>

            {/* CSS-only hover via .page-hover-card */}
            <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
              {topExecutors.map((executor, i) => (
                <Link key={executor.id} href={`/profile/${executor.id}`} className="page-hover-card">
                  {i < 3 && (
                  ...
                  <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-white text-2xl font-extrabold mb-3 shadow-md"
                  ...
                  <div className="flex justify-center mb-2"><Stars rating={executor.rating} /></div>
                  ...
                  <div className="mt-4 text-xs font-bold gradient-text">Посмотреть профиль →</div>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="py-16 px-4" style={{ background: "#f8fff8" }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
            {TRUST.map((t) => (
              <div key={t.label} className="page-hover-card" style={{ cursor: "default" }}>
                <div className="w-12 h-12 mx-auto mb-3 rounded-2xl flex items-center justify-center text-2xl"
      ...
      <section className="py-20 px-4 relative overflow-hidden"
        ...
        <div className="relative max-w-xl mx-auto text-center">
          ...
          <h2 className="text-4xl font-extrabold text-white mb-4 tracking-tight">Попробуйте прямо сейчас</h2>
          <p className="text-white/80 mb-8 text-base leading-relaxed">
            Первое поручение за 30 секунд. Регистрация бесплатная.<br />Оплата только после выполнения.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/register"
              className="inline-flex items-center justify-center gap-2 bg-white text-[#14A800] font-bold py-3.5 px-8 text-base hover:bg-green-50 hover:-translate-y-px transition-all"
              style={{ borderRadius: "20px", boxShadow: "0 4px 16px rgba(0,0,0,.15)" }}>
              Создать аккаунт бесплатно
            </Link>
            <Link href="/tasks"
              className="inline-flex items-center justify-center gap-2 font-semibold py-3.5 px-8 text-base text-white hover:-translate-y-px transition-all"
              style={{ background: "rgba(255,255,255,.15)", border: "1px solid rgba(255,255,255,.30)", borderRadius: "20px", backdropFilter: "blur(8px)" }}>
              Смотреть задачи
            </Link>
          </div>
        </div>
